...
const name='TestFinale30',key=name.toLowerCase(),quote=s=>"'"+s.replaceAll("'","''")+"'";
...
try{
 ...
 const p={...g.freshCookiePlayer(Date.now()+60000),lifetime:g.COOKIE_FINALE_TOTAL*(1-1e-12),balance:1e140,runEarned:1e140,achievements:g.COOKIE_ACHIEVEMENTS.map(a=>a.id)};
 db('UPDATE cookie_players SET data='+quote(JSON.stringify(p))+',version=version+1 WHERE author_key='+quote(key));
 check('final profile blocked below finale',(await avatar()).status===400);
 check('final companion blocked below finale',(await req({kind:'companionEquip',companion:138,revision:0})).status===400);
 db("UPDATE cookie_players SET data=json_set(data,'$.lifetime',"+g.COOKIE_FINALE_TOTAL+"),version=version+1 WHERE author_key="+quote(key));
 ...
 check('final companion accepted at finale',equipped.status===200&&equipped.player.voyages.equippedCompanion===138);
 const profile=await avatar();check('final profile accepted at finale',profile.status===201&&profile.record.avatar===138);
 const reb=await req({kind:'prestige'});check('full tree and finale survive prestige',reb.status===200&&g.masterySpent(reb.player)===27&&reb.player.lifetime===g.COOKIE_FINALE_TOTAL&&g.companionRecord(reb.player,138).stage===3);
