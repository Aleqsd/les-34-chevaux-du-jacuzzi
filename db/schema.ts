...
export const cookiePlayers=sqliteTable("cookie_players",{authorKey:text("author_key").primaryKey(),author:text("author").notNull(),data:text("data").notNull(),version:integer("version").notNull().default(0),updated:integer("updated").notNull(),lastAction:text("last_action").notNull().default("")});
export const cookieActions=sqliteTable("cookie_actions",{authorKey:text("author_key").notNull(),id:text("id").notNull(),payload:text("payload").notNull(),result:text("result").notNull(),created:integer("created").notNull()},t=>[primaryKey({columns:[t.authorKey,t.id]}),index("idx_cookie_actions_created").on(t.created)]);
