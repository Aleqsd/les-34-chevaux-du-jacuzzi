...
export function buildingPrice(p:CookiePlayer,index:number,quantity:number){const b=BUILDINGS[index];return Math.ceil(b.price*Math.pow(1.15,p.buildings[index])*(Math.pow(1.15,quantity)-1)/.15*(p.specialization==="architect"?.92:1));}
...
export const manualClickPower=(p:CookiePlayer,now=Date.now())=>clickPower(p,now)*(p.specialization==="artisan"?1.25:1);
export const offlineHours=(p:CookiePlayer)=>p.specialization==="watcher"?12:8;
...
export function contractTerms(p:CookiePlayer,kind:ContractKind){
 const referenceRate=Math.max(1,baseProduction(p)+autoClickRate(p.clicks)*clickPower({...p,rushUntil:0},0));
...
export function canInvest(p:CookiePlayer){return BUILDINGS.some((_,i)=>p.buildings[i]<1000&&buildingPrice(p,i,1)<=CAP)||UPGRADES.some(u=>!p.upgrades.includes(u.id)&&upgradeReady(p,u));}
...
export function settle(p:CookiePlayer,now:number){ensureRebuild(p);const end=Math.max(p.updated,now),elapsed=Math.min(end-p.updated,offlineHours(p)*3600000),start=p.updated,boosted=Math.max(0,Math.min(start+elapsed,p.rushUntil)-start);const gain=baseProduction(p)*(elapsed+boosted*6)/1000;earn(p,gain);contractProgress(p,"produce",gain);p.clickCredit=Math.min(25,p.clickCredit+(end-p.updated)*.025);const autoRate=autoClickRate(p.clicks);if(autoRate||p.autoCredit!==undefined)p.autoCredit=Math.min(autoRate*2,(p.autoCredit??0)+(end-p.updated)*autoRate/1000);p.updated=end;award(p);return gain;}
...
export type CookieAction={kind:"sync"}|{kind:"click";count:number}|{kind:"auto";count:number}|{kind:"event";eventAt:number}|{kind:"buy";building:number;quantity:number}|{kind:"upgrade";upgrade:string}|{kind:"golden"}|{kind:"mission";mission:string}|{kind:"rebuild";mission:string;run:number}|{kind:"contractAccept";contract:ContractKind;cycle:number}|{kind:"contractClaim";contractId:number}|{kind:"contractCancel";contractId:number}|{kind:"specialize";specialization:Specialization;expectedChangeAt:number}|{kind:"prestige"};
export function applyCookieAction(p:CookiePlayer,action:CookieAction,now:number){const offline=settle(p,now);let acceptedClicks=0,eventReward=0,rebuildBonus=0,contractBonus=0,eventEffect:"cookies"|"rush"|"golden"|undefined;
 if(action.kind==="click"){acceptedClicks=Math.min(action.count,Math.floor(p.clickCredit));p.clickCredit-=acceptedClicks;p.clicks+=acceptedClicks;const gain=acceptedClicks*manualClickPower(p,now);earn(p,gain);contractProgress(p,"produce",gain);}
 ...
 award(p);return {offline,acceptedClicks,...(rebuildBonus?{rebuildBonus}:{}),...(contractBonus?{contractBonus}:{}),...(eventEffect?{eventReward,eventEffect}:{})};
}
