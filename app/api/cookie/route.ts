...
const action=z.discriminatedUnion("kind",[z.object({kind:z.literal("sync")}),z.object({kind:z.literal("click"),count:z.number().int().min(1).max(25)}),z.object({kind:z.literal("auto"),count:z.number().int().min(1).max(25)}),z.object({kind:z.literal("buy"),building:z.number().int().min(0).max(9),quantity:z.union([z.literal(1),z.literal(10),z.literal(100)])}),z.object({kind:z.literal("upgrade"),upgrade:z.string().max(40)}),z.object({kind:z.literal("golden")}),z.object({kind:z.literal("mission"),mission:z.string().max(10)}),z.object({kind:z.literal("prestige")})]);
