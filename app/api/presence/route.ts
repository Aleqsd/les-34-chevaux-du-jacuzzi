...
const payload=z.object({sessionId:z.string().uuid(),author:z.string().trim().min(1).max(40),room:z.enum(["villa","cinema","terrace","jacuzzi"]),action:z.enum(["walk","sit","bathe","dance","wave"]),revision:z.number().int().min(0).max(1000000000)});
...
export async function POST(request:Request){try{
  ...
  const p=payload.parse(JSON.parse(raw)),now=Date.now(),db=database(),action=p.action==="wave"||p.action==="dance"?p.action:p.room==="jacuzzi"?"bathe":p.action==="bathe"?"walk":p.action;
