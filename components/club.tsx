...
async function api<T>(url: string, options?: RequestInit): Promise<T> {
  let r:Response;
  try{r=await fetch(url,options);}catch(error){if((error as Error).name==="AbortError")throw error;if((error as Error).name==="TimeoutError")throw new Error("La sauvegarde n’a pas répondu à temps. Réessaie dans un instant.");throw new Error("Connexion interrompue. Vérifie ta connexion puis réessaie.");}
