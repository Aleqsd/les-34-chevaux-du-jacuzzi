...
import { JacuzziScene } from "@/components/jacuzzi-scene";
import { CrewCards, Discussion, ActivityPractical, MovieDuel } from "@/components/crew-social";
...
import { CREW, EMPTY_STATE, dateLabel, score, timeLabel, tripDays, type ClubState, type Movie, type Proposal } from "@/lib/club";
...
export default function Club() {
  ...
  const chooseName = (name:string) => {name=name.trim();if(!name)return;setPerson(name);setNameDraft(name);try{localStorage.setItem("jacuzzi-prenom",name);}catch{}setIdentityOpen(false);toast.success(`Bienvenue au QG, ${name} !`);const action=pendingAction.current;pendingAction.current=null;if(action)setTimeout(()=>action(name),150);};
  ...
  const retain=(p:Proposal)=>{setDetail(null);setActivityDetail(null);setSelectPlan(p);};
  ...
  const vote = (id:string,value:number,slot=false) => identify(async author=>{
    if(pendingVotes.includes(id))return;setPendingVotes(v=>[...v,id]);
    try{await mutate({action:"vote",id:crypto.randomUUID(),[slot?"slotId":"proposalId"]:id,author,value});celebrate(value===1?"yes":"no");toast.success(value===1?`${author} est chaud ! Vote ajouté.`:`C’est noté, ${author}. Vote contre ajouté.`);}
    catch(e){toast.error((e as Error).message);}finally{setPendingVotes(v=>v.filter(x=>x!==id));}
  });
  ...
  const openActivity = (date?:string,idea?:{title:string;url:string}) => {setSlotFor(null);setTitle(idea?.title||"");setUrl(idea?.url||"");setFormError("");setDay(date || (idea ? "" : "2026-09-24"));setStart(idea ? "" : "14:00");setEnd(idea ? "" : "16:00");setActivityOpen(true);};
  const openSlot = (p:Proposal) => {setActivityDetail(null);setSlotFor(p);setDay(p.start?localDay(p.start):"2026-09-24");setStart("17:00");setEnd("19:00");setFormError("");setActivityOpen(true);};
  const saveActivity = (e:FormEvent) => {
    ...
    identify(async author=>{setSaving(true);try{await mutate(slotFor?{action:"slot",proposalId:slotFor.id,author,...times}:{action:"propose",kind:"activity",title:submittedTitle,url:submittedUrl,author,...times});setActivityOpen(false);celebrate();toast.success(slotFor?"Un autre créneau à départager !":"Le programme prend forme. Activité ajoutée !");}catch(err){setFormError((err as Error).message);}finally{setSaving(false);}});
  };
  ...
  const votedNames=new Set(state.votes.map(v=>v.author));
    ...
    <Tabs value={view} onValueChange={goTo} className="app-shell">
      ...
      <main>
        ...
        <TabsContent value="lobby" className="view-panel lobby-panel">
          <section className="lobby-hero"><div className="lobby-copy"><span className="eyebrow"><span className="live-dot"/> LE QG EST OUVERT · 20—27 SEPT.</span><h1>34 CHEVAUX.<br/><em>ZÉRO FREIN.</em></h1><p>11 potes. Un jacuzzi. Des idées beaucoup trop bonnes pour rester dans le bain.</p><div className="lobby-actions"><button className="button primary" onClick={()=>goTo("cinema")}><Clapperboard size={18}/>Entrer au cinéma<ArrowUpRight size={18}/></button><button className="button secondary" onClick={()=>goTo("activities")}><CalendarDays size={18}/>Voir le programme<ArrowUpRight size={18}/></button></div><span className="lobby-coordinate">MAGALAS, FRANCE<br/>43° N · LES VACANCES EN GRAND</span></div><JacuzziScene/><span className="lobby-orbit" aria-hidden="true">LE BAIN DES LÉGENDES ✳</span></section>
          {loadError&&<div className="error-banner" role="alert">{loadError}<button onClick={()=>void load()}>Réessayer</button></div>}
          <div className="lobby-strip"><span><b>{films.length}</b> films à départager</span><span><b>{activities.length}</b> plans à vivre</span><span><b>{state.votes.length+state.duelVotes.length}</b> votes exprimés</span><button onClick={()=>identify(()=>setAvatarOpen(true))}><CrewAvatar name={person||"Alex"} small/>Personnaliser mon avatar<ArrowUpRight size={16}/></button></div>
        ...
        <TabsContent value="cinema" className="view-panel">
          ...
          <MovieDuel {...social} onSelect={retain}/>
        ...
        <TabsContent value="activities" className="view-panel">
          ...
          <section className="planning-section"><div className="section-heading"><div><div className="eyebrow muted">LE PROGRAMME DES RÉJOUISSANCES</div><h2>Une semaine ensemble<span className="yellow-star">✳</span></h2></div><span className="date-pill"><CalendarDays size={16}/>20 — 27 sept. 2026</span></div><div className="planning-meta"><span>{activities.length} activité{activities.length!==1?"s":""} proposée{activities.length!==1?"s":""}</span><span>Heure de Paris · Créneaux ouverts au débat</span></div>
          {loadError&&<div className="error-banner" role="alert">{loadError}<button onClick={()=>void load()}>Réessayer</button></div>}
          <div className="week-scroll"><div className="week-grid">{tripDays.map((d,i)=>{const plans=activities.filter(p=>p.start&&localDay(p.start)===d).sort((a,b)=>a.start!.localeCompare(b.start!));const today=localDay(new Date().toISOString())===d;return <div className={`day-column ${today?"today":""}`} key={d}><div className="day-heading"><span>{dateLabel(d,{weekday:"short"}).replace(".","")}</span><strong>{20+i}</strong>{today?<small>AUJOURD’HUI</small>:<small>SEPTEMBRE</small>}</div><div className="day-events">{plans.length?plans.map(p=><button key={p.id} className="day-event" onClick={()=>setActivityDetail(p.id)}><span>{timeLabel(p.start!)} — {timeLabel(p.end!)}</span><strong>{p.title}</strong><span className="event-author">{p.author}<ArrowUpRight size={14}/></span></button>):<div className="free-day"><Sun size={20}/><span>Quartier libre</span></div>}</div><button className="day-add" onClick={()=>openActivity(d)} aria-label={`Proposer une activité le ${dateLabel(d)}`}><Plus size={17}/><span>Une idée ?</span></button></div>;})}</div></div></section>
          <section className="activity-section"><div className="section-heading"><div><div className="eyebrow muted">ON EN DIT QUOI ?</div><h2>Les plans du crew<span className="count">{activities.length.toString().padStart(2,"0")}</span></h2></div></div>{activities.length?<div className="activity-grid">{activities.map((p,i)=>{const slots=state.slots.filter(s=>s.proposalId===p.id);return <article className="activity-card enter" key={p.id} style={{"--delay":`${i*60}ms`,"--card-accent":COLORS[i%COLORS.length]} as CSSProperties}><div className="activity-top"><span className="activity-number">{String(i+1).padStart(2,"0")}</span><span className="activity-day">{p.start?dateLabel(p.start,{weekday:"short",day:"numeric"}):""}</span><ArrowUpRight size={25}/></div><h3><button onClick={()=>setActivityDetail(p.id)}>{p.title}</button></h3><div className="activity-time"><Clock3 size={16}/>{p.start&&timeLabel(p.start)} — {p.end&&timeLabel(p.end)}</div><div className="proposed"><Avatar name={p.author} small/><span>Une idée de <strong>{p.author}</strong></span></div>{Votes({id:p.id})}<button className="discussion-link" onClick={()=>setActivityDetail(p.id)}>Infos & discussion · {state.comments.filter(c=>c.proposalId===p.id).length} message(s)<ArrowUpRight size={14}/></button><div className="activity-actions"><a href={p.url} target="_blank" rel="noopener noreferrer">Voir le lieu<ExternalLink size={14}/></a><button onClick={()=>openSlot(p)}>Autre créneau<Plus size={15}/></button></div>{slots.length>0&&<button className="alternatives-count" onClick={()=>setActivityDetail(p.id)}>{slots.length} autre{slots.length>1?"s":""} créneau{slots.length>1?"x":""} proposé{slots.length>1?"s":""}<ArrowRight size={15}/></button>}</article>;})}</div>:<div className="activity-empty"><Sparkles size={35}/><h3>Tout reste à inventer.</h3><p>Une rando, un resto, un escape game ?<br/>Un nom, un lien, un créneau. Le crew décide.</p><button className="button secondary" onClick={()=>openActivity()}>Lancer la première idée<Plus size={17}/></button></div>}</section>
          <section className="activity-ideas" aria-labelledby="activity-ideas-title">
            ...
            <div className="activity-ideas-grid">{ACTIVITY_IDEAS.map((idea,i)=><article className={`idea-card idea-${idea.theme} enter`} key={idea.theme} onPointerMove={tilt} onPointerLeave={untilt} style={{"--delay":`${i*80}ms`} as CSSProperties}>
              ...
              <div className="idea-copy"><h3>{idea.name}</h3><p>{idea.description}</p></div>
