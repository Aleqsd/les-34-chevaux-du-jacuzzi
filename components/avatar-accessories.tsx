...
import { EYEWEAR, HATS, type Look } from "@/lib/appearance";
...
export function accessoryLabel(kind:AccessoryKind,look:Look){return kind==="hat"?HATS.find(h=>h.id===look.hat)?.label||"Chapeau":kind==="eyewear"?EYEWEAR.find(h=>h.id===look.eyewear)?.label||"Lunettes":"Bouée";}
export function visibleAccessories(look:Look){return ACCESSORIES.filter(kind=>kind==="hat"?look.hat!=="none":kind==="eyewear"?look.eyewear!=="none":!!look.floatie);}
...
export function AvatarAccessories({look,editing}:{look:Look;editing?:AccessoryEditing}){
  ...
  const start=(event:PointerEvent<HTMLButtonElement>,kind:AccessoryKind)=>{
    ...
    const {x,y}=accessoryLayout(kind,look.hat,look.positions);
  ...
  return visibleAccessories(look).map(kind=>{
    const layout=accessoryLayout(kind,look.hat,look.positions),emoji=kind==="hat"?HATS.find(h=>h.id===look.hat)?.emoji:kind==="eyewear"?EYEWEAR.find(h=>h.id===look.eyewear)?.emoji:"🛟";
    ...
    const content=<span className="accessory-glyph" aria-hidden="true">{emoji}</span>;
