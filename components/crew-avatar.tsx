...
export const AVATAR_NAMES=["Cheval","Chat","Renard","Panda","Axolotl","Lapin","Loutre","Pingouin","Dragon","Capybara","Grenouille","Licorne"];
...
export function CrewAvatar({name,small=false,index,className=""}:{name:string;small?:boolean;index?:number;className?:string}){
  const profiles=useContext(AvatarContext);const profile=profiles.find(p=>p.authorKey===key(name));const i=index??avatarIndex(name,profiles);const [failed,setFailed]=useState(false);useEffect(()=>setFailed(false),[profile?.imageUrl]);
  return <span title={name} className={`avatar kawaii-avatar ${small?"small":""} ${className}`} style={{backgroundPosition:`${(i%4)/3*100}% ${Math.floor(i/4)/2*100}%`} as CSSProperties}>{index===undefined&&profile?.imageUrl&&!failed&&<img src={profile.imageUrl} alt="" referrerPolicy="no-referrer" onError={()=>setFailed(true)}/>}</span>;
}
